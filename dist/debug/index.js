/* Copyright(c) 2023 SendBird, Inc. SendBird Desk JavaScript SDK v1.2.0 */
import { DeviceOsPlatform, SendbirdPlatform, SendbirdProduct } from '@sendbird/chat';
...
class Config {
    ...
    /**
     * @ignore
     * @desc Resolve the API host. Priority: customApiHost > hostType-based host.
     *       Type-based hosts get `/sapi` auto-appended; customApiHost is used as-is.
     */
    static get apiHost() {
        if (_customApiHost) {
            return _customApiHost;
        }
        const appId = _sendbird.appId;
        switch (_hostType) {
            case ApiHostType.DELIGHT:
                return `https://desk-api-${appId}.app.delight.ai/sapi`;
            case ApiHostType.DEFAULT:
            default:
                return `https://desk-api-${appId}.sendbird.com/sapi`;
        }
    }
    /**
     * @ignore
     * @desc Set the API host type. null/undefined falls back to DEFAULT.
     */
    static set hostType(type) {
        if (type) {
            _hostType = type;
        }
        else {
            _hostType = ApiHostType.DEFAULT;
        }
    }
    /**
     * @ignore
     * @desc Set a fully custom API host (used as-is, trailing `/` removed).
     *       null/empty clears it and reverts to the hostType-based host.
     */
    static set customApiHost(host) {
        if (host) {
            _customApiHost = host.replace(/\/+$/, '');
        }
        else {
            _customApiHost = null;
        }
...
class Auth {
     ...
     */
    static async connect(userId, accessToken, language) {
...
class Ticket {
    ...
    fetchFromJSON(params) {
        ...
        this.firstResponseTime = params.firstResponseTime ?? 0;
    ...
    static async _getTicketList(params) {
        ...
        if (Array.isArray(data.results)) {
            ...
            Logger.write(`[REQ] ${params.status ?? 'ALL'} ticket list:`, tickets);
        }
        return tickets;
    }
    /**
...
function parseAuthArgs(params) {
    ...
    if (params[1] !== undefined) {
        const typeOfSecondParam = typeof params[1];
        if (typeOfSecondParam === 'function') {
            // connect(userId, cb);
            result.cb = params[1];
        }
        else if (typeOfSecondParam === 'string') {
            // connect(userId, accessToken);
            result.accessToken = params[1];
        }
    }
    if (params[2] !== undefined) {
        const typeOfThirdParam = typeof params[2];
        if (typeOfThirdParam === 'function') {
            // connect(userId, accessToken, cb);
            result.cb = params[2];
        }
        else if (typeOfThirdParam === 'string') {
            // connect(userId, accessToken, language);
            result.language = params[2];
        }
    }
    if (params[3] !== undefined) {
        const typeOfFourthParam = typeof params[3];
        if (typeOfFourthParam === 'function') {
            // connect(userId, accessToken, language, cb);
            result.cb = params[3];
        }
    }
    return result;
...
function validateAuthArgs(params) {
    const { userId, accessToken, language, cb } = params;
    ...
    if (language !== undefined && typeof language !== 'string') {
        Logger.write('Language to SendbirdDesk.authenticate(userId, accessToken, language) must be a string');
        throw SendbirdDeskError.create(SendbirdDeskError.Type.ERROR_INVALID_PARAMETER);
    }
    if (typeof cb !== 'function') {
...
class SendbirdDesk {
    ...
    static init(sendbird, platformOrParams) {
        let params = {};
        if (typeof platformOrParams === 'string') {
            params = { platform: platformOrParams };
        }
        else if (platformOrParams) {
            params = platformOrParams;
        }
        Config.sendbird = sendbird;
        if (params.platform) {
            Config.platform = params.platform;
        }
        Config.hostType = params.hostType;
        _initialized = true;
    }
    static authenticate(...params) {
        const { userId, accessToken, language, cb } = parseAuthArgs(params);
        validateAuthArgs({ userId, accessToken, language, cb });
        this._authenticate({ userId, accessToken, language, cb }).finally(() => {
            cb();
        });
    }
    /**
    ...
    static async _authenticate(params) {
        const { userId, accessToken, language } = params;
        if (_initialized) {
            try {
                await Auth.connect(userId, accessToken, language);
            }
            catch (err) {
    ...
    static setCustomerCustomFields(customFields, cb) {
        this._setCustomerCustomFields(customFields)
            .then((res) => {
            cb(res, null);
        })
            .catch((err) => {
     ...
     * */
    static setCustomerLanguage(language, callback) {
        this._setCustomerLanguage(language)
            .then(() => {
            callback(null);
        })
            .catch((err) => {
    ...
    static async _setCustomerLanguage(language) {
        if (typeof language !== 'string' || language.length === 0) {
            throw SendbirdDeskError.create(SendbirdDeskError.Type.ERROR_INVALID_PARAMETER);
        }
        const res = await fetch(`${Config.apiHost}/customers/language/`, {
            method: 'PATCH',
            headers: Auth.header,
            body: JSON.stringify({
                ...Auth.postParam,
...
export { Agent, ApiHostType, RelatedChannel, SendbirdDeskError, Ticket, SendbirdDesk as default };
